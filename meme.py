...
async def fetch(account_line):
    ...
    log("❌ Max retries reached. Moving to the next task.", red)
    return None
...
async def check_user(index):
    ...
    if response.status_code == 200:
        ...
        if 'errors' in response_data:
            log(f"❌ Error: Invalid Query ID", red)
            return None
        else:
...
async def activate_energy_recharge_booster(index):
    ...
    if response.status_code == 200:
        ...
        log(f"❌ Encountered error with status {response.status_code}, retrying...", red)
        return None
...
async def activate_booster(index):
            ...
            for _ in range(10):
                ...
                for retry in range(3):
                    ...
                    if tap_result is None:
                        log(f"❌ Error tapping, retrying {retry + 1} with {total_hit//2} taps...", yellow)
                        total_hit //= 2
                        if retry == 2:
                            log("❌ Unable to tap after 3 retries. Moving to the next loop.", red)
                    ...
                    if isinstance(tap_result, dict) and 'data' in tap_result and 'telegramGameProcessTapsBatch' in tap_result['data']:
                        ...
                        if tap_data['currentBoss']['currentHealth'] == 0:
                            log("Boss defeated, moving to the next boss...", yellow)
                            await set_next_boss(index)
                        log(f"Tapping memefi: {tap_data['coinsAmount']}, Boss ⚔️: {tap_data['currentBoss']['currentHealth']} - {tap_data['currentBoss']['maxHealth']}", green)
                        break
                    else:
    ...
    log("Turbo Boost activation process finished", cyan)
...
async def submit_taps(index, json_payload):
    try:
        ...
        if response.status_code == 200:
            ...
            log(f"❌ Failed with status {response.status_code}, retrying...", red)
            return None
    except Exception as e:
        log(f"Error: {e}", red)
        return None
...
async def set_next_boss(index):
    ...
    if response.status_code == 200:
        ...
        if response_data and 'data' in response_data:
            log("Next boss successfully set!", green)
            return response_data
        else:
            log("❌ Unable to set next boss: Incomplete or missing data.", red)
            return None
    else:
        log(f"❌ Encountered error with status {response.status_code}, retrying...", red)
        return None
...
async def main():
    log("Starting Memefi bot...", magenta)
        ...
        with open('data.txt', 'r') as file:
            ...
            for index, line in enumerate(account_lines):
                ...
                if result is not None:
                    ...
                    log(f"❌ Account {index + 1}: Invalid token or error occurred", red)
            ...
            for index, result, first_name, last_name in accounts:
                ...
                if 'currentBoss' in user_data:
                    ...
                    if lv_boss >= 14 and boss_health == 0:
                        log(f"=================== {first_name} {last_name} END ====================", magenta)
                        should_continue_to_next_account = True
                    if boss_health == 0:
                        log("Boss defeated, moving to next boss...", yellow)
                        await set_next_boss(index)
                log("Starting taps", green)
                ...
                if not should_continue_to_next_account:
                    while current_energy > 500 or recharge_available > 0:
                        ...
                        if tap_result is not None:
                            ...
                            current_energy = user_data['currentEnergy']
                            recharge_available = user_data['freeBoosts']['currentRefillEnergyAmount']
                            log(f"Tapping Memefi2 : Balance 💎 {user_data['coinsAmount']} Energy: {current_energy} / {user_data['maxEnergy']}", green)
                        else:
                            log(f"❌ Error with status {tap_result}, retrying...", red)

                        if current_energy < 500:
                            if recharge_available > 0:
                                log("Low energy, activating Recharge...", yellow)
